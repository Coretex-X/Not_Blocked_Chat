...
def main_menu(page):
    ...
    user_data = get_user_data(db_path)

    dlg = create_exit_dialog()
    confirm_dialog = create_delete_chat_dialog()

    # 6 значений: +content_container для динамической ширины
    contact_dialog, contact_list, search_field, loading_container, search_result_container, contact_content = create_contact_dialog()
    ...
    def _dialog_width():
        """Ширина диалога = ширина экрана минус 32px отступы"""
        w = page.width or 400
        return max(260, w - 32)
            ...
            favorites_container.controls.append(
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text("Долго нажмите ⋮ рядом с чатом, чтобы добавить",
    ...
    def open_new_chat_dialog(e):
        # Задаём ширину под текущий экран перед открытием
        contact_content.width = _dialog_width()
